...
public abstract class OMCOffHandPassivePower extends OMCPassivePower{
	...
	public OMCOffHandPassivePower(String registryName) {
		super(registryName);
	}
	
	public abstract OMCItem getItem();
	public abstract void onOffhandHoldingTick(Player p, int ticker, ItemStack it);
	public abstract void onOffhandStopHolding(Player p);
	public abstract void onOffhandStartHolding(Player p);
	...
	public void onTick(int ticker) {
		...
		for(Player p : Bukkit.getOnlinePlayers()) {
			boolean holding = PlayerConditions.holdsItemInOffHand(p, omcItem);
			boolean wasHolding = HOLDING.contains(p.getUniqueId());
			if(holding) {
				if(!wasHolding) onOffhandStartHolding(p);
				onOffhandHoldingTick(p, ticker, p.getInventory().getItemInOffHand());
				NEW_HOLDING.add(p.getUniqueId());
			} else if(wasHolding) onOffhandStopHolding(p);
		}
		HOLDING = NEW_HOLDING;
	}
}
